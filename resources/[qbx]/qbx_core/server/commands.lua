...
-- lib.addCommand('extras', {
--     help = locale('command.extras.help'),
--     restricted = 'group.admin'
-- }, function(source)
--     TriggerClientEvent("nos:client:activateVehicleExtras", source)
-- end)
...
lib.addCommand('givemoney', {
    ...
    -- Max 1 000 000$
    if args['amount'] > 1000000 then
        Notify(source, "Le montant est trop élevé.", 'error')
        return
    end

    local adminName = GetPlayer(source).PlayerData.name
    local joueurName = player.PlayerData.name
