...
Config.DeathScreenType = 4
-- If you want to use the previous death screen(Draw text, not recommended) see docs

Config.ShowEMSCountOnDeath = false -- Show active ambulance count in death UI? (Gives count in death screen of all jobs active from Config.ambulanceJobs)
...
Config.ambulanceJobs = {       -- Jobs that are considered ambulance jobs (If unsure, likely leave the way it is)
    'ems',               -- You must have a job with this name registered
...
Config.NassPaintball = {
    ...
    ------------------------------------
    autoDetect = false -- Auto detect if player is in paintball area and disable injury/death system
}
...
Config.FuelSystem = 'ox_fuel'       -- 'legacy' (LegacyFuel) / 'ox' (ox_fuel) / 'default'
...
Config.lowHealthAlert = {
    ...
    notification = {
        title = 'ATTENTION',
        description = 'Tu te sens faible, tu devrais aller voir un médecin.',
    }
}
    ...
    Pillbox = {
        ...
        clockInAndOut = {
            ...
            target = {
                ...
                coords = vec3(313.25, -593.90002441406, 43.2),
                heading = 337.07,
                width = 1.0,
                length = 1.0,
                minZ = 43.28 - 0.9,
