...
function OpenPersonalStash(station)
    ...
    local maxWeight = Config.Locations[station].personalLocker.maxWeight or 50000
    local slots = Config.Locations[station].personalLocker.slots or 50
...
function OpenEvidenceLocker(station)
    ...
    -- start Sélection de casier au lieu de l'input
    local input = wsb.inputDialog(
        Strings.evidence_storage,
        {
            {
                type = 'select',
                label = 'Choisir casier de saisies',
                options = {
                    { label = 'Drogues', value = '1' },
                    { label = 'Armes', value = '2' },
                    { label = 'Effets personnels', value = '3' },
                    { label = 'Fourre-tout', value = '999' }
                }
            }
        }
    )
    -- end Sélection de casier au lieu de l'input
