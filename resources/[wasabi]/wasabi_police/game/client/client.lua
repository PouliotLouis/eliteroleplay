...
AddEventHandler('wasabi_police:spawnVehicle', function(data)
    ...
    if string.sub(model, -1) == 'b' then
        model = string.sub(model, 1, -2)
        unmarked = true
    end

    local category = Config.Locations[data.station].vehicles.options[data.grade][data.model].category
    local spawnLocs = Config.Locations[data.station].vehicles.spawn[category]
    local extras = Config.Locations[data.station].vehicles.options[data.grade][data.model].extras
    local livery = Config.Locations[data.station].vehicles.options[data.grade][data.model].livery
    local plateCiv = Config.Locations[data.station].vehicles.options[data.grade][data.model].plateCiv
    local plateIndex = Config.Locations[data.station].vehicles.options[data.grade][data.model].plateIndex
    local spawnLoc = nil
    ...
    if not IsModelInCdimage(GetHashKey(model)) then
        ...
        for i, sl in pairs(spawnLocs) do
            local nearbyVehicles = wsb.getNearbyVehicles(vec3(sl.coords.x, sl.coords.y, sl.coords.z), 2.5, true)
...
AddEventHandler('wasabi_police:toggleDuty', function()
    local job, grade = wsb.hasGroup(Config.policeJobs)
    if not job or not grade then return end
    TriggerEvent('wasabi_police:client:checkDuty', nil, true)
    ...
    for k, v in pairs(Config.Locations) do
        ...
        if v.vehicles.enabled then
            CreateThread(function()
                ...
                for i, _ in pairs(v.vehicles.spawn.air) do
                    local airPoint = wsb.points.new({
                        coords = v.vehicles.spawn.air[i].coords,
                        distance = 10,
                        nearby = function(self)
                            if not self.isClosest or (self.currentDistance > v.clockInAndOut.distance) then return end
                            local hasJob
